...
<head>
    ...
    <title>{{ __('Estimate') }} {{ $estimate->estimate_no }} — {{ $company->name }}</title>
    <style>
...
<body>
    @php
        $billLines = collect([
            $estimate->contact?->display_name,
            $estimate->contact?->billing_line1,
            $estimate->contact?->billing_line2,
            collect([$estimate->contact?->billing_city, $estimate->contact?->billing_region, $estimate->contact?->billing_postal_code])->filter()->implode(', '),
        ])->filter()->values();

        $shipLines = collect([
            $estimate->contact?->shipping_line1,
            $estimate->contact?->shipping_line2,
            collect([$estimate->contact?->shipping_city, $estimate->contact?->shipping_region, $estimate->contact?->shipping_postal_code])->filter()->implode(', '),
        ])->filter()->values();
    @endphp
        ...
        <tr>
            ...
            <td style="width: 45%;">
                ...
                <table class="meta">
                    ...
                    <tr>
                        <td>{{ $estimate->estimate_date?->format('n/j/Y') }}</td>
    ...
    <table class="meta full" style="margin-top: 14px;">
        ...
        <tr>
            <td>{{ optional($estimate->terms)->name ?? '—' }}</td>
            <td>{{ $estimate->expires_on?->format('n/j/Y') ?? '—' }}</td>
            <td>{{ $estimate->customer_po ?: '—' }}</td>
            <td>{{ optional($estimate->salesRep)->display_name ?? '—' }}</td>
        </tr>
    </table>
    ...
    <table class="totals">
        <tr>
            <td>{{ __('Subtotal') }}</td>
            <td class="num">{{ number_format($estimate->subtotal_cents / 100, 2) }}</td>
        </tr>
        ...
        <tr class="grand">
            <td>{{ __('Total') }}</td>
            <td class="num">${{ number_format($estimate->total_cents / 100, 2) }}</td>
        </tr>
    </table>

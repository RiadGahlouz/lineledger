...
<body>
    @php
        $billLines = collect([
            $salesOrder->contact?->display_name,
            $salesOrder->contact?->billing_line1,
            $salesOrder->contact?->billing_line2,
            collect([$salesOrder->contact?->billing_city, $salesOrder->contact?->billing_region, $salesOrder->contact?->billing_postal_code])->filter()->implode(', '),
        ])->filter()->values();

        $shipLines = collect([
            $salesOrder->contact?->shipping_line1,
            $salesOrder->contact?->shipping_line2,
            collect([$salesOrder->contact?->shipping_city, $salesOrder->contact?->shipping_region, $salesOrder->contact?->shipping_postal_code])->filter()->implode(', '),
        ])->filter()->values();
    @endphp
    ...
    <table class="full top">
        <tr>
            ...
            <td style="width: 45%;">
                <h1 class="title">{{ __('SALES ORDER') }}</h1>
                <table class="meta">
                    <tr>
                        <th>{{ __('Date') }}</th>
                        <th>{{ __('Order #') }}</th>
                    ...
                    <tr>
                        <td>{{ $salesOrder->order_date?->format('n/j/Y') }}</td>
    ...
    <table class="meta full" style="margin-top: 14px;">
        ...
        <tr>
            <td>{{ optional($salesOrder->terms)->name ?? '—' }}</td>
            <td>{{ $salesOrder->expected_date?->format('n/j/Y') ?? '—' }}</td>
            <td>{{ $salesOrder->customer_po ?: '—' }}</td>
            <td>{{ optional($salesOrder->salesRep)->display_name ?? '—' }}</td>
        </tr>
    </table>

    @if ($salesOrder->ship_date || $salesOrder->ship_via || $salesOrder->fob || $salesOrder->tracking_no)
        <table class="meta full" style="margin-top: 8px;">
            ...
            <tr>
                <td>{{ $salesOrder->ship_date?->format('n/j/Y') ?? '—' }}</td>
                <td>{{ $salesOrder->ship_via ?: '—' }}</td>
                <td>{{ $salesOrder->fob ?: '—' }}</td>
                <td>{{ $salesOrder->tracking_no ?: '—' }}</td>
            </tr>
        </table>
    @endif
    ...
    <table class="totals">
        <tr>
            <td>{{ __('Subtotal') }}</td>
            <td class="num">{{ number_format($salesOrder->subtotal_cents / 100, 2) }}</td>
        </tr>
        ...
        <tr class="grand">
            <td>{{ __('Total') }}</td>
            <td class="num">${{ number_format($salesOrder->total_cents / 100, 2) }}</td>
        </tr>
    </table>

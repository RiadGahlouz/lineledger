...
<body>
    @php
        $vendorLines = collect([
            $purchaseOrder->contact?->display_name,
            $purchaseOrder->contact?->billing_line1,
            $purchaseOrder->contact?->billing_line2,
            collect([$purchaseOrder->contact?->billing_city, $purchaseOrder->contact?->billing_region, $purchaseOrder->contact?->billing_postal_code])->filter()->implode(', '),
        ])->filter()->values();
    ...
    <table class="full top">
        <tr>
            ...
            <td style="width: 45%;">
                <h1 class="title">{{ __('PURCHASE ORDER') }}</h1>
                <table class="meta">
                    <tr>
                        <th>{{ __('Date') }}</th>
                        <th>{{ __('PO #') }}</th>
                    ...
                    <tr>
                        <td>{{ $purchaseOrder->po_date?->format('n/j/Y') }}</td>
    ...
    <table class="meta full" style="margin-top: 14px;">
        ...
        <tr>
            <td>{{ optional($purchaseOrder->terms)->name ?? '—' }}</td>
            <td>{{ $purchaseOrder->expected_date?->format('n/j/Y') ?? '—' }}</td>
        </tr>
    </table>
    ...
    <table class="totals">
        <tr>
            <td>{{ __('Subtotal') }}</td>
            <td class="num">{{ number_format($purchaseOrder->subtotal_cents / 100, 2) }}</td>
        </tr>
        ...
        <tr class="grand">
            <td>{{ __('Total') }}</td>
            <td class="num">${{ number_format($purchaseOrder->total_cents / 100, 2) }}</td>
        </tr>
    </table>
    ...
    <div class="footer">
        ...
        @endif
        @if (filled($purchaseOrder->vendor_message))
            <div class="message">{{ $purchaseOrder->vendor_message }}</div>
        @endif

...
new #[Title('Purchase Order')] class extends Component
{
     ...
     *     unit_price: string, discount_pct: string, tax_code_id: ?int,
     *     secondary_tax_code_id: ?int, class_id: ?int, location_id: ?int,
    ...
    public function mount(Company $company, ?PurchaseOrder $purchaseOrder = null): void
    {
        ...
        if ($purchaseOrder && $purchaseOrder->exists) {
            abort_unless($purchaseOrder->effectiveStatus()->isEditable(), 403, 'This purchase order can no longer be edited.');

            $this->purchaseOrder = $purchaseOrder->load('lines');
            $this->contact_id = $purchaseOrder->contact_id;
            $this->po_no = $purchaseOrder->po_no;
            $this->po_date = $purchaseOrder->po_date->toDateString();
            $this->expected_date = $purchaseOrder->expected_date?->toDateString() ?? '';
            $this->terms_id = $purchaseOrder->terms_id;
            $this->ship_to = $purchaseOrder->ship_to ?? '';
            $this->memo = $purchaseOrder->memo ?? '';
            $this->vendor_message = $purchaseOrder->vendor_message ?? '';
            ...
            $this->po_date = $this->company->currentDateTime()->toDateString();
            $this->po_no = app(DocumentNumberGenerator::class)->next($company, PurchaseOrder::class, 'po_no', 'PO');
            $this->lines = [$this->emptyLine()];
        }
    }

    /**
     * @return array{item_id: ?int, account_id: ?int, description: string, quantity: string, unit_price: string, discount_pct: string, tax_code_id: ?int, secondary_tax_code_id: ?int, class_id: ?int, location_id: ?int, subtotal: int, tax: int, secondary_tax: int, total: int}
     */
    ...
    /**
     * Triggered when an item is picked — prefill the line with its purchase defaults.
     */
    ...
    public function save(): void
    {
        ...
        $validated = $this->validate([
            ...
            'ship_to' => ['nullable', 'string', 'max:120'],
        ...
        $this->purchaseOrder = app(SavePurchaseOrder::class)->handle([
        ...
        Flux::toast(variant: 'success', text: __('Purchase order saved.'));
        $this->redirectRoute('purchase-orders.show', ['company' => $this->company->slug, 'purchaseOrder' => $this->purchaseOrder->id], navigate: true);
    }
    ...
    {
        // Expense, asset and liability accounts are codeable on a purchase line.
        // Keep any account already coded on a line visible even if deactivated.
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <x-contact-combo
                ...
                data-test="purchase-order-vendor-combo"
            />

            <flux:input wire:model="po_no" :label="__('PO #')" required data-test="purchase-order-no-input" />
            <flux:input type="date" wire:model="po_date" :label="__('Date')" required data-test="purchase-order-date-input" />
            <flux:input type="date" wire:model="expected_date" :label="__('Expected date')" data-test="purchase-order-expected-input" />
            ...
            <flux:input wire:model="ship_to" :label="__('Ship to')" data-test="purchase-order-ship-to" />
        </div>

        <flux:input wire:model="memo" :label="__('Memo')" />
        <flux:textarea wire:model="vendor_message" :label="__('Vendor message')" :description="__('Shown to the vendor on the printed purchase order.')" rows="2" />
            ...
            >
                ...
                <tbody class="lg:divide-y lg:divide-border">
                    @foreach ($lines as $i => $line)
                        <tr wire:key="line-{{ $i }}" data-test="purchase-order-line-row" class="block border-b border-border p-3 lg:table-row lg:border-0 lg:p-0">
                ...
                <tfoot class="hidden bg-muted lg:table-footer-group">
                    <tr>
                        ...
                        <td class="px-2 py-2 text-right font-mono" data-test="purchase-order-subtotal">{{ number_format($this->totals['subtotal'] / 100, 2) }}</td>
                        <td></td>
                    </tr>
                    @forelse ($this->taxBreakdown as $taxRow)
                        <tr data-test="purchase-order-tax-row">
                            <td colspan="{{ 6 + $this->dimensionColumns }}" class="px-2 py-2 text-right font-medium">{{ $taxRow['label'] }} {{ LineTaxBreakdown::formatRate($taxRow['rate']) }}%</td>
                    ...
                    @empty
                        <tr>
                            ...
                            <td class="px-2 py-2 text-right font-mono" data-test="purchase-order-tax">{{ number_format($this->totals['tax'] / 100, 2) }}</td>
                            <td></td>
                        </tr>
                    @endforelse
                    <tr class="text-base">
                        <td colspan="{{ 6 + $this->dimensionColumns }}" class="px-2 py-2 text-right font-semibold">{{ __('Total') }}</td>
                        <td class="px-2 py-2 text-right font-mono font-semibold" data-test="purchase-order-total">{{ number_format($this->totals['total'] / 100, 2) }}</td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
